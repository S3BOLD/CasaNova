...
export async function editGift(id, updates) {
  const gifts = await readGifts();
  let found = false;
  ...
  const next = gifts.map((g) => {
    if (g.id !== id) return g;
    found = true;
    const maxClaims = Number(updates.maxClaims);
    return {
      ...g,
      name: String(updates.name || g.name).slice(0, 60),
      category: String(updates.category || g.category),
      desc: String(updates.desc ?? g.desc ?? '').slice(0, 140),
      maxClaims: Number.isFinite(maxClaims) && maxClaims > 0 ? Math.floor(maxClaims) : g.maxClaims || 1,
    };
  });

  if (!found) {
    const err = new Error('Presente não encontrado.');
    err.code = 'NOT_FOUND';
    throw err;
  }
